...
$$
ce_{i} = \log(\hat{y}_{i}) - \log(y_{i}) = \log\left(\frac{\hat{y}_{i}}{y_{i}}\right) 
$$
$$
\exp(ce_{i}) = \frac{\hat{y}_{i}}{y_{i}}
$$
...
price_changed_data_df <- data_df %>% 
  filter(price_change) %>% 
  mutate(multiplicative_causal_effect = multiplicative_causal_effect)
...
causal_effect <- price_changed_data_df %>%
  mutate(causal_effect = Sales / multiplicative_causal_effect) %>% 
